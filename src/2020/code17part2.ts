...
class Position4 {
  ...
  get key() : PositionKey4 {
    return `${this.x}|${this.y}|${this.z}|${this.w}`;
  }
}

// eacy element in this will contain an iteration
var iterations = new Array<Set<PositionKey4>>();
var firstIteration = new Set<PositionKey4>();
...
var zbounds = [0, 0];
var wbounds = [0, 0];
const totalCycles4 = 6;
for(var cycle = 1 ; cycle <= totalCycles4 ; cycle++) {
  ...
  var iteration = new Set<PositionKey4>();
  iterations.push(iteration);

  for(var x = xbounds[0]; x <= xbounds[1] ; x++) {
    for(var y = ybounds[0]; y <= ybounds[1] ; y++) {
      for(var z = zbounds[0]; z <= zbounds[1] ; z++) {
        for(var w = wbounds[0]; w <= wbounds[1] ; w++) {
          
          // check all ?? adjacent spots in prev cycle and count how many we have
          const prevCycle = iterations[cycle - 1];
          const currentlyActive = prevCycle.has(new Position4(x, y, z, w).key);
            ...
            for(var cy = -1 ; cy <= 1 ; cy++) {
              for(var cz = -1 ; cz <= 1 ; cz++) {
                for(var cw = -1 ; cw <= 1 ; cw++) {
                  ...
                  neighboursActive += prevCycle.has(new Position4(x + cx, y + cy, z + cz, w + cw).key) ? 1 : 0;
                }
              }
            }
          }

          //console.log(`(${x},${y},${z},${w} - currently:${currentlyActive} num active:${neighboursActive}`)
          ...
          if(active) {
            iteration.add(new Position4(x, y, z, w).key);
          }
